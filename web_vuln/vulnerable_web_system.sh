...
# =================================================================
# VULNERABLE WEB SYSTEM - ENHANCED HTML IMPLEMENTATION
# =================================================================
...
# Log file
LOG_DIR="/var/log/vulnerable-web-system"
...
run_command() {
    ...
    log_info "Menjalankan: $description"
...
setup_logging() {
    log_info "Menyiapkan sistem logging"
    mkdir -p "$LOG_DIR"
    touch "$LOG_FILE" "$ERROR_LOG"
    chmod 644 "$LOG_FILE" "$ERROR_LOG"
    log_success "Sistem logging siap"
}

# Fungsi untuk instalasi dependencies
install_dependencies() {
    log_info "Menginstalasi dependencies"
    ...
    log_success "Instalasi dependencies selesai"
}

# Fungsi untuk konfigurasi Apache multi-site
configure_apache_multisite() {
    log_info "Mengkonfigurasi Apache multi-site"
...
EOF
    ...
    log_success "Konfigurasi Apache multi-site selesai"
}

# Fungsi untuk membuat Admin Panel Enhanced
create_admin_panel() {
    log_info "Membuat Admin Panel Enhanced"
    ...
    if [ -f "admin_panel_enhanced.html" ]; then
        ...
        log_warning "File admin_panel_enhanced.html tidak ditemukan, menggunakan template default"
...
EOF
    ...
    log_success "Admin Panel Enhanced dibuat"
}

# Fungsi untuk membuat Government Website Enhanced
create_government_website() {
    log_info "Membuat Government Website Enhanced"
    ...
    if [ -f "government_website_enhanced.html" ]; then
        ...
        log_warning "File government_website_enhanced.html tidak ditemukan, menggunakan template default"
...
EOF
    ...
    log_success "Government Website Enhanced dibuat"
}
...
create_sensitive_files() {
    log_info "Membuat file sensitive"
...
EOF
    ...
    log_success "File sensitive dibuat"
}
...
test_installation() {
    ...
    if curl -s -o /dev/null -w "%{http_code}" "http://localhost:$APACHE_PORT_ADMIN" | grep -q "200"; then
        log_success "Admin Panel berjalan di port $APACHE_PORT_ADMIN"
    else
        log_error "Admin Panel tidak berjalan di port $APACHE_PORT_ADMIN"
    fi
    
    # Test Government Website
    if curl -s -o /dev/null -w "%{http_code}" "http://localhost:$APACHE_PORT_GOV" | grep -q "200"; then
        log_success "Government website berjalan di port $APACHE_PORT_GOV"
    else
        log_error "Government website tidak berjalan di port $APACHE_PORT_GOV"
    fi
    
    # Test Admin Panel login page
    if curl -s "http://localhost:$APACHE_PORT_ADMIN/login.php" | grep -q "Admin Login"; then
        log_success "Halaman login Admin Panel tersedia"
    else
        log_error "Halaman login Admin Panel tidak tersedia"
    fi
    
    # Test Government admin panel
    if curl -s "http://localhost:$APACHE_PORT_GOV/admin/login.php" | grep -q "Admin Login"; then
        log_success "Admin panel Government website tersedia"
    else
        log_error "Admin panel Government website tidak tersedia"
    fi
    
    log_success "Testing instalasi selesai"
}

# Fungsi untuk menampilkan informasi instalasi
show_installation_info() {
    log_info "Menampilkan informasi instalasi"
...
# Main function
main() {
    echo -e "${BLUE}=== VULNERABLE WEB SYSTEM - ENHANCED HTML IMPLEMENTATION ===${NC}"
    ...
    log_info "Memulai instalasi vulnerable web system"
    ...
    log_success "Instalasi vulnerable web system selesai!"
    log_info "Log file tersedia di: $LOG_FILE"
    
    if [ -s "$ERROR_LOG" ]; then
        log_warning "Terdapat error selama instalasi. Cek: $ERROR_LOG"
    fi
}
